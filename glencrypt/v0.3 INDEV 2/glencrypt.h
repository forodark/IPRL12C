...
//#define showMenu(title, ...) do { \
//    printf("%s:\n", title); \
//    showMenuOptions(__VA_ARGS__, NULL); \
//} while(0)
//
//void showMenuOptions(const char* option, ...) {
//    va_list args;
...
void showMenuOptions(const char* title, const char* const* options, int numOptions) {
    printf("%s:\n", title);

    int startIndex = 0;
    int endIndex = MAX_OPTIONS_PER_PAGE;

    while (startIndex < numOptions) {
        for (int i = startIndex; i < endIndex && i < numOptions; i++) {
            printf("[%d] %s\n", i + 1, options[i]);
        }
        ...
        if (choice >= 1 && choice <= numOptions) {
            ...
            printf("Exiting...\n");
            break;
        } else if (choice == endIndex + 1 && endIndex < numOptions) {
            startIndex = endIndex;
            endIndex += MAX_OPTIONS_PER_PAGE;
        } else if (choice == startIndex && startIndex > 0) {
            endIndex = startIndex;
            startIndex -= MAX_OPTIONS_PER_PAGE;
        } else {
...
void showMenu(const char* title, ...) {
    const char* options[MAX_OPTIONS_PER_PAGE * 10];  // Maximum of 10 pages
    ...
    while (option != NULL && numOptions < MAX_OPTIONS_PER_PAGE * 10) {
    ...
    showMenuOptions(title, options, numOptions);
}
